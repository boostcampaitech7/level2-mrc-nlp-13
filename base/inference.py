...
from dense_encoder import BertEncoder, RobertaEncoder
...
def main():
    ...
    if data_args.retriever_type == 'dense':
    ...
    elif data_args.retriever_type == 'sparse':
        if data_args.eval_retrieval:
        ...
        if data_args.eval_retrieval:
            datasets = run_hybrid_retrieval(
                model_args, tokenizer.tokenize, datasets, training_args, data_args, data_args.hybrid_score_option
            )
        print('Retriever Part End...')
        torch.cuda.empty_cache()
        # eval or predict mrc model
...
def run_hybrid_retrieval(
    ...
    retriever = HybridRetrieval(
        ...
        data_path=data_path, 
        context_path=context_path,
        option= data_args.hybrid_score_option
    )
