...
# from QA.korquad_qg.config import QGConfig
# from QA.korquad_qg.dataset import (MAX_QUESTION_SPACE, MIN_QUESTION_SPACE, QGDecodingDataset, load_wiki_dataset)

# model = GPT2LMHeadModel.from_pretrained("taeminlee/kogpt2")
# model.load_state_dict(torch.load('QA/model/QG_kogpt2.pth', map_location="cpu"))
# device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
# model = model.to(device)

# tokenizer = SentencePieceBPETokenizer.from_file(
#     vocab_filename="QA/tokenizer/vocab.json", merges_filename="QA/tokenizer/merges.txt", add_prefix_space=False
# )
...
import sys
sys.path.append('/data/ephemeral/home/sungeun/level2-mrc-nlp-13/code/augmented_data')
from QA.korquad_qg.config import QGConfig
from QA.korquad_qg.dataset import (MAX_QUESTION_SPACE, MIN_QUESTION_SPACE, QGDecodingDataset, load_wiki_dataset)
...
examples_list = load_wiki_dataset('/data/ephemeral/home/sungeun/level2-mrc-nlp-13/code/augmented_data/merge_wiki_text_title.csv')
random.shuffle(examples_list)
examples=[]
d_id=[]
for i in examples_list:
