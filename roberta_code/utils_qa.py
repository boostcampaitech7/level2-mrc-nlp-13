...
def normalize_answer(text):
    # 구두점 제거
    text = re.sub(r'[^\w\s]', '', text)  # 한국어와 숫자를 제외한 모든 구두점 제거
    # 불필요한 공백 제거
...
def postprocess_qa_predictions(
	...
	null_score_diff_threshold: float = 0.4,
	...
	for example_index, example in enumerate(tqdm(examples)):
		...
		# 중복된 text 제거 및 상위 n-best 크기로 제한
		unique_offsets = set()  # 중복을 체크할 기준으로 offsets 사용
		deduped_predictions = []
		for pred in predictions:
			if pred['offsets'] not in unique_offsets:  # offsets를 중복 체크 기준으로 사용
				deduped_predictions.append(pred)
				unique_offsets.add(pred['offsets'])
    
		# 중복된 답변이 몇 개 제거되었는지 로깅
		# logger.info(f"Filtered out {len(predictions) - len(deduped_predictions)} duplicate answers.")

		# 중복 제거된 예측에서 다시 상위 n-best만 남김
		predictions = sorted(
			deduped_predictions, key=lambda x: x["score"], reverse=True
		)[:n_best_size]
		...
		# 정규화 적용
		# for pred in predictions:
		# 	pred["text"] = normalize_answer(pred["text"])
		...
		if not version_2_with_negative:
			...
			if score_diff > null_score_diff_threshold:
				...
				# null 답변이 아닌 최상의 답변 선택 로직
				best_non_empty_pred = None
				for pred in predictions:
					if len(pred["text"]) > 1:  # 너무 짧은 답변 제외
						best_non_empty_pred = pred
						break
				
				if best_non_empty_pred:
					all_predictions[example["id"]] = best_non_empty_pred["text"]
				else:
					all_predictions[example["id"]] = ""
